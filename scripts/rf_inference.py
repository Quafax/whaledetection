...
from whaledetection.model.random_forest import load_model, predict
...
def preprocess_audio(wav_path, cfg):
    signal, sr = librosa.load(wav_path, sr=cfg.loadSignal.sr, mono=True)
...
def main():
    if len(sys.argv) < 2:
        raise SystemExit("Usage: python scripts/rf_inference.py <audio.wav>")

...
#import noisereduce to test spectral gating vs swt denoising
import noisereduce as nr

# configuration variables
base_dir = "C:/Users/luede/Seafile/WhaleData" #direction of species folders
sr = 16384   #sampling rate to use idea: 16384
...
y_labels = []
file_paths = []
k_factors_mode = "energy" #None, energy, or default (factor for level based thresholding, none is default decreasing with level (because lower frequencies have more energy usually))
...
def swt_denoise(x, level, wavelet):
    x=zero_pad(x, level)
...
#################test of spectral gating denoising##############
...
def denoise_signal_percentile (signal, wavelet, level, percentile):
  ...
  for j, (cA_j, cD_j) in enumerate(coeffs, start=1):
    ...
    thr = np.percentile(np.abs(cD_j), percentile)
